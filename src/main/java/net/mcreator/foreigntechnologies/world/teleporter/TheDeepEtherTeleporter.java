...
public class TheDeepEtherTeleporter implements ITeleporter {
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("the_deep_ether_portal", Vec3i::compareTo, 300);
	public static PoiType poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegistryEvent.Register<PoiType> event) {
		poi = new PoiType("the_deep_ether_portal",
				com.google.common.collect.Sets.newHashSet(
						ImmutableSet.copyOf(ForeignTechnologiesModBlocks.THE_DEEP_ETHER_PORTAL.get().getStateDefinition().getPossibleStates())),
				0, 1).setRegistryName("the_deep_ether_portal");
	...
	public TheDeepEtherTeleporter(ServerLevel worldServer, BlockPos entityEnterPos) {
				...
				for (int j3 = 0; j3 < 2; ++j3) {
					for (int k3 = -1; k3 < 3; ++k3) {
						BlockState blockstate1 = k3 < 0
								? ForeignTechnologiesModBlocks.ETHER_SLATE_BRICK.get().defaultBlockState()
		...
		for (int l1 = -1; l1 < 3; ++l1) {
			for (int j2 = -1; j2 < 4; ++j2) {
				if (l1 == -1 || l1 == 2 || j2 == -1 || j2 == 3) {
					...
					this.level.setBlock(blockpos$mutableblockpos, ForeignTechnologiesModBlocks.ETHER_SLATE_BRICK.get().defaultBlockState(), 3);
				}
			}
		}
	...
	private PortalInfo getPortalInfo(Entity entity, ServerLevel server) {
		...
		return this.getExitPortal(entity, blockpos1, worldborder).map(repositioner -> {
			...
			if (blockstate.hasProperty(BlockStateProperties.HORIZONTAL_AXIS)) {
				...
				vector3d = TheDeepEtherPortalShape.getRelativePosition(teleportationrepositioner$result, direction$axis, entity.position(),
						entity.getDimensions(entity.getPose()));
			} else {
				direction$axis = Direction.Axis.X;
				vector3d = new Vec3(0.5, 0, 0);
			}
			return TheDeepEtherPortalShape.createPortalInfo(server, repositioner, direction$axis, vector3d, entity.getDimensions(entity.getPose()),
